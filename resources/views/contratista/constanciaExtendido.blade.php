...
<html>
    <head>
    <meta charset="utf-8"/>
    <title>{{ $empresa->rfc_empresa }}</title>
...
<body>
    <div class="container">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td style="text-align:right; font-size:10px;">
                                {{ $empresa->motivo_empresa == '1' ? 'CONSTANCIA DE INSCRIPCIÓN': 'CONSTANCIA DE REFRENDO' }}
            ...
            <div class="row">
                <div class="col-12">
                    <b>{{ $empresa->tipo == '1' ? mb_strtoupper($empresa->nombre_empresa) : mb_strtoupper($empresa->nombre_persona) }}</b><br />
                    RFC: <b>{{ $empresa->rfc_empresa }}</b><br/>
                    REPRESENTANTE LEGAL: <b>{{ $empresa->tipo == '1' ? mb_strtoupper($empresa->representantes->nombre_representante) : mb_strtoupper($empresa->nombre_persona) }}</b><br/>
                    {{ mb_strtoupper($empresa->domicilio) }}<br/>
                    COL. {{ mb_strtoupper($empresa->colonia) }}, CP. {{ strtoupper($empresa->codigo_postal) }}.<br/>
                    {{ mb_strtoupper($empresa->municipio->nombre ) }}, {{ strtoupper($empresa->estado->abreviatura ) }}
                </div>
            </div>


            <div class="row primer-texto">
                <div class="text-block-3" style="font-size:12px;">En atención a su Solicitud y en virtud de haber acreditado lo dispuesto en el artículo 31 de La Ley de Obras Públicas y sus Servicios del Estado de Guerrero número 266, hace de su conocimiento que la <i>Secretaría de Desarrollo Urbano, Obras Públicas Y Ordenamiento Territorial</i>, ha tenido a bien {{ $empresa->motivo_empresa == '2' ? 'Refrendarlo' : 'Registrarlo' }} en el Padrón Único de Contratistas del Gobierno del Estado de Guerrero, con el número <b>{{ $empresa->folio->folio }}</b>, y de acuerdo al artículo 30 de la ley en mención, se le asignan la (s) especialidad (es) número(s):</div>
            </div>
            ...
            <div class="row" style="margin-top: 20px; font-size:12px;">
                <ul>
                    ...
                    @foreach ($empresa->especialidades as $especialidad)
                            ...
                            @if(strlen($especialidad->especialidad->nombre) > 35 && $empresa->especialidades()->count() > 4)
                                <li><b><span style="color: #ab0033">&#9679;</span> {{ substr($especialidad->especialidad->nombre, 0, 3) }}</b><span style="font-size: 11px;">{{ substr($especialidad->especialidad->nombre, 3) }}</span></li>
                            @else
            ...
            <div style="font-size:12px; text-align: justify;">Así mismo, se le recuerda que con fundamento en el articulo 32 de la ley en comento, el registro en el Padrón Único de Contratistas del Estado de Guerrero tendrá <i>Vigencia Indefinida</i>, siempre y cuando se cumpla con la obligación de Refrendarlo anualmente.</div>

            <div style="margin-top: {{ $empresa->especialidades()->count() > 6 ? 15 : 60 }}px;font-size:14px; text-align: center;">
                ...
                <b style="font-size:16px;">ARQ. URB. IRENE JIMÉNEZ MONTIEL</b>
            </div>
            ...
            <footer>
                ...
                <div class="row">
                    ...
                    <div class="col-3" style="text-align: center;">
                        <!-- <img class="qr" src='data:image/png;base64, {!! base64_encode(QrCode::format('png')->merge('https://puc.guerrero.gob.mx/constancia/jaguar.png', .2, true)->size(120)->generate('https://puc.guerrero.gob.mx/validacion/'.$empresa->folio->uuid.'/'.$empresa->rfc_empresa)) !!} '> -->
                        <img class="qr" src='data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(120)->generate('https://puc.guerrero.gob.mx/validacion/'.$empresa->folio->uuid.'/'.$empresa->rfc_empresa)) !!} '>
                    </div>
                </div>
