...
$(function () {
	...
	var table = $('#dataTableUsuarios').DataTable({
                ajax: "{{ route('administracion.usuarios.list') }}",
                serverSide: true,
                processing: true,
                columns: [
                    {data: 'avatar', name: 'avatar', orderable: false, searchable: false, exportable: false},
                    {data: 'rol', name: 'rol'},
                    {data: 'rfc', name: 'rfc'},
                    {data: 'name', name: 'name'},
            ...
            $("body").on("click","#password",function(event){
                ...
                Swal.fire({
                    title: 'ATENCIÓN !!!',
                    text: "Cambiara la contraseña del Usuario.",
                    input: 'text',
                    reverseButtons: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    preConfirm: (password) => {
                        return fetch(`usuarios/password/`+id+`/${password}`)
                        .then(response => {
                            if (!response.ok) {
							...
							<div class="widget widget-stats bg-teal">
								...
								<div class="stats-content">
									...
									<div class="stats-link">
										<a href="/registro" class="btn btn-primary">Nuevo <i class="fa fa-arrow-alt-circle-right"></i></a>
									</div>
								</div>
							</div>
						</div>
