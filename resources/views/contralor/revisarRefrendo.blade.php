...
$(function () {
    ...
    @else
        ...
        $('#validar').on('click',function(e){
            ...
            var id = {{ $refrendo->id }};
            var contraloria_id = $('#contraloria_id').val();
            var empresa_id = $('#empresa_id').val();
            var refrendo_id = $('#refrendo_id').val();
            var contraloria =  myEditor.getData();
        ...
        <div class="col-xl-12">
            ...
            <div class="panel panel-inverse" data-sortable-id="index-1">
                ...
                <div class="panel-body">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <textarea name="obras" id="editorObras" placeholder="{{ isset($refrendo->observacionesRefrendos->obras)? '' : 'No ha hecho alguna Observación' }}">

                                {{isset($refrendo->observacionesRefrendos->obras)? $refrendo->observacionesRefrendos->obras : '' }}
                    ...
                    <form method="POST" action="{{ route('contralor.refrendo.observaciones') }}" autocomplete="off" id="formEnviar">
                        @csrf
                        <input type="hidden" class="form-control" id="empresa_id" name="empresa_id" value="{{ $refrendo->empresa->id }}" readonly />
                        <input type="hidden" class="form-control" id="contraloria_id" name="contraloria_id" value="{{ auth()->user()->id }}" readonly />
                        <input type="hidden" class="form-control" id="refrendo_id" name="refrendo_id" value="{{ $refrendo->id }}" readonly />

                        <div class="row">
                            <div class="d-flex">
                                <h3>Observaciones para el Área de Padrón:</h3>
                            </div>
                        </div>

                        <div class="row">
                            <div class="col-md-12">
                                <textarea name="contraloria" id="editorContralor" placeholder="{{ isset($refrendo->observacionesRefrendos->contraloria)? '' : 'No ha hecho alguna Observación' }}">

                                    {{isset($refrendo->observacionesRefrendos->contraloria)? $refrendo->observacionesRefrendos->contraloria : '' }}
                                    ...
                                    @if($refrendo->estatus != 'O' )
                                            ...
                                            <script>
                                                window.addEventListener("load", (e)=>{
                                                    ClassicEditor.create( document.querySelector('#editorContralor' ),{
                                                    } )
                                                    .then( editor => {
                                                        console.log( editor );
                                                        myEditor = editor;
                                    ...
                                    @else
                                            <script>
                                                window.addEventListener("load", (e)=>{
                                                    ...
                                                    .then(editor => {
                                                        editor.isReadOnly; // `false`.
                                                            editor.enableReadOnlyMode( '#editorContralor' );
                                                            const toolbarElement = editor.ui.view.toolbar.element;
                                                            toolbarElement.style.display = 'none';
                                                    } ) .catch( error => {
                                                        console.error( error );
                            ...
                            <div class="row" style="margin-top: 15px;">
                                <div class="d-flex justify-content-end">
                                    ...
                                    @else
                                        <p><b>La empresa se encuentra corrigiendo sus datos (OBSERVACIONES) cuando sean envidas para revisón CONTRALORIA podrá emitir OBSERVACIONES o VALIDARLA.</b></p>
                                    @endif
